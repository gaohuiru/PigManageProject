        ...
        <div class="tpl-portlet-components">
            <div class="portlet-title">
                <div class="caption font-green bold">
                    <span class="am-icon-code"></span> 用户登录日志
                </div>
                ...
                <div class="am-g">
                    ...
                    <div class="am-u-sm-12 am-u-md-3">
                        <div class="am-form-group">
                            ...
                            <option value="ip"
                                    <c:if test="${result['queryCondition']=='q(ip)p'}">selected</c:if>
                            >IP
                            </option>
                            <option value="time"
                                    <c:if test="${result['queryCondition']=='q(time)p'}">selected</c:if>
                            >时间
                            <option value="if_succeed"
                                    <c:if test="${result['queryCondition']=='q(if_succeed)p'}">selected</c:if>
                            >登录状态
                        ...
                        <div class="am-input-group am-input-group-sm">
                            ...
                            <span class="am-input-group-btn">
                                ...
                                <button class="am-btn  am-btn-default am-btn-success tpl-am-btn-success am-icon-search"
                                        type="button"
                                        value="/log/queryLoginLogs?operationId=${sessionScope.id}&service=queryLoginLogs&pageNo=1&pageSize=${result['pageSize']}"
                                        onclick="search(this)"></button>
                                <button class="am-btn  am-btn-default am-btn-default tpl-am-btn-default am-icon-refresh"
                                        type="button" value="/log/queryLoginLogs?operationId=${sessionScope.id}&service=queryLoginLogs"
...
<%--                                <th class="table-date am-hide-sm-only">操作</th>--%>
                            ...
                            <tbody>
                            <c:forEach items="${result['pageInfo'].list}" var="login">
                                <tr>
                                    <td><label>
                                        <input type="checkbox">
                                    </label></td>
                                    ...
                                    <td><c:if test="${login.ifSucceed==-1}">登录异常</c:if>
                                        <c:if test="${login.ifSucceed==0}">账号或密码错误</c:if>
                                        <c:if test="${login.ifSucceed==1}">登录成功</c:if>
                                    </td>
                                    <td>${login.time}</td>
                                    <td title="${login.msg}"><div class="my-table-wrap">${login.msg}</div></td>
                                    <%--删除按钮--%>
<%--                                    <td>--%>
<%--                                        <div class="am-btn-toolbar">--%>
<%--                                            <div class="am-btn-group am-btn-group-xs">--%>
<%--                                                <div class="am-modal am-modal-confirm" tabindex="-1"--%>
<%--                                                     id="login-remove-modal-${login.id}">--%>
<%--                                                    <div class="am-modal-dialog">--%>
<%--                                                        <div class="am-modal-hd">删除确认</div>--%>
<%--                                                        <form action="${pageContext.request.contextPath}/log/removeLoginLog?operatorId=${sessionScope.id}&service=removeLoginLog&id=${login.id}&pageNo=${result['pageNo']}&pageSize=${result['pageSize']}&queryCondition=${result['queryCondition']}&queryValue=${result['queryValue']}"--%>
<%--                                                              method="post"--%>
<%--                                                              id="login-remove-form-${login.id}">--%>
...
<%--                                                                <span class="am-modal-btn"--%>
<%--                                                                      onclick="document.getElementById('login-remove-form-${login.id}').submit();return false;">确定</span>--%>
<%--                                                            </div>--%>
<%--                                                        </form>--%>
<%--                                                    </div>--%>
<%--                                                </div>--%>
<%--                                                <button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only"--%>
<%--                                                        data-am-modal="{target: '#login-remove-modal-${login.id}'}">--%>
                            ...
                            <div class="am-fr">
                                ...
                                <ul class="am-pagination tpl-pagination">
                                    ...
                                    <c:if test="${pageNumberStart!=1}">
                                        <li>
                                            <a href="${pageContext.request.contextPath}/log/queryLoginLogs?operationId=${sessionScope.id}&service=queryLoginLogs&pageNo=${pageNumberStart-1}&pageSize=${result['pageSize']}&queryCondition=${result['queryCondition']}&queryValue=${result['queryValue']}">«</a>
                                        </li>
                                    </c:if>
                                    ...
                                    <c:forEach begin="${pageNumberStart}" end="${pageNumberEnd}"
                                        ...
                                        <li
                                            ...
                                            <a href="${pageContext.request.contextPath}/log/queryLoginLogs?operationId=${sessionScope.id}&service=queryLoginLogs&pageNo=${pageNo}&pageSize=${result['pageSize']}&queryCondition=${result['queryCondition']}&queryValue=${result['queryValue']}">${pageNo}</a>
                                        </li>
                                    </c:forEach>
                                    <%--按此按钮去往下一组页码的第一个--%>
                                    <c:if test="${pageNumberEnd!=result['pageInfo'].totalPage}">
                                        <li>
                                            <a href="${pageContext.request.contextPath}/log/queryLoginLogs?operationId=${sessionScope.id}&service=queryLoginLogs&pageNo=${pageNumberEnd+1}&pageSize=${result['pageSize']}&queryCondition=${result['queryCondition']}&queryValue=${result['queryValue']}">»</a>
                                        </li>
                                    </c:if>
