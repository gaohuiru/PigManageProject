                        ...
                        <div class="am-input-group am-input-group-sm">
                            ...
                            <span class="am-input-group-btn">
                                ...
                                <button class="am-btn  am-btn-default am-btn-success tpl-am-btn-success am-icon-search"
                                        type="button"
                                        value="/log/queryErrorLogs?operationId=${sessionScope.id}&service=queryErrorLogs&pageNo=1&pageSize=${result['pageSize']}"
                                        onclick="search(this)"></button>
                                <button class="am-btn  am-btn-default am-btn-default tpl-am-btn-default am-icon-refresh"
                                        type="button"
                                        value="/log/queryErrorLogs?operationId=${sessionScope.id}&service=queryErrorLogs"
                ...
                <div class="am-g">
                    <div class="am-u-sm-12">
                        ...
                        <table class="am-table am-table-striped am-table-hover table-main">
                            ...
                            <tbody>
                            <c:forEach items="${result['pageInfo'].list}" var="error">
                                <tr>
                                    <td><label>
                                        <input type="checkbox">
                                    </label></td>
...
<%--                                                     id="error-remove-modal-${error.id}">--%>
<%--                                                    <div class="am-modal-dialog">--%>
<%--                                                        <div class="am-modal-hd">删除确认</div>--%>
<%--                                                        <form action="${pageContext.request.contextPath}/log/removeErrorLog?operatorId=${sessionScope.id}&service=removeErrorLog&id=${error.id}&pageNo=${result['pageNo']}&pageSize=${result['pageSize']}&queryCondition=${result['queryCondition']}&queryValue=${result['queryValue']}"--%>
<%--                                                              method="post"--%>
<%--                                                              id="error-remove-form-${error.id}">--%>
...
<%--                                                                <span class="am-modal-btn"--%>
<%--                                                                      onclick="document.getElementById('error-remove-form-${error.id}').submit();return false;">确定</span>--%>
<%--                                                            </div>--%>
<%--                                                        </form>--%>
<%--                                                    </div>--%>
<%--                                                </div>--%>
<%--                                                <button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only"--%>
<%--                                                        data-am-modal="{target: '#error-remove-modal-${error.id}'}">--%>
                            ...
                            <div class="am-fr">
                                ...
                                <ul class="am-pagination tpl-pagination">
                                    ...
                                    <c:if test="${pageNumberStart!=1}">
                                        <li>
                                            <a href="${pageContext.request.contextPath}/log/queryErrorLogs?operationId=${sessionScope.id}&service=queryErrorLogs&pageNo=${pageNumberStart-1}&pageSize=${result['pageSize']}&queryCondition=${result['queryCondition']}&queryValue=${result['queryValue']}">«</a>
                                        </li>
                                    </c:if>
                                    ...
                                    <c:forEach begin="${pageNumberStart}" end="${pageNumberEnd}"
                                        ...
                                        <li
                                            ...
                                            <a href="${pageContext.request.contextPath}/log/queryErrorLogs?operationId=${sessionScope.id}&service=queryErrorLogs&pageNo=${pageNo}&pageSize=${result['pageSize']}&queryCondition=${result['queryCondition']}&queryValue=${result['queryValue']}">${pageNo}</a>
                                        </li>
                                    </c:forEach>
                                    <%--按此按钮去往下一组页码的第一个--%>
                                    <c:if test="${pageNumberEnd!=result['pageInfo'].totalPage}">
                                        <li>
                                            <a href="${pageContext.request.contextPath}/log/queryErrorLogs?operationId=${sessionScope.id}&service=queryErrorLogs&pageNo=${pageNumberEnd+1}&pageSize=${result['pageSize']}&queryCondition=${result['queryCondition']}&queryValue=${result['queryValue']}">»</a>
                                        </li>
                                    </c:if>
